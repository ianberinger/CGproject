  ...
  for (int i = 0; i < 50; i++) {
    for (int j = 0; j < 50; j++) {
      switch (m[i][j]) {
        ...
        case 3: {
          std::shared_ptr<Entity> p(new Road(i, 0, j, 1, 1, 1, 0.0, 0.0, true,
...
bool recurse(map &m, int case1X, int case1Z, int case2X, int case2Z,
  ...
  if (m[case1X][case1Z] == 4) {
    defineTrack(m, case1X, case1Z, nextDirection(direction, true), entities,
                generateRoad);
    return true;
  } else if (m[case2X][case2Z] == 4) {
    // go right
    defineTrack(m, case2X, case2Z, nextDirection(direction, false), entities,
                generateRoad);
    return true;
  }
...
void defineTrack(map &m, int startX, int startZ, Direction direction,
  ...
  printf("New barrier at X:%d Z:%d length:%d direction:%s\n", startX, startZ,
         length, mods[direction].name.c_str());
  addBarrier(startX, startZ, length, direction, entities);
  if (generateRoad) {
    addRoad(m, startX, startZ, direction, entities);
  }
  ...
  if (recurse(m, case1X, case1Z, case2X, case2Z, direction, entities,
              generateRoad)) {
    return;
  }
  ...
  if (!recurse(m, case1X, case1Z, case2X, case2Z, direction, entities,
               generateRoad)) {
    printf(
        "Map ambiguous at X:%d Z:%d case1X:%d case1Z:%d case2X:%d case2Z:%d\n",
...
void addRoad(map &m, int x, int z, Direction direction,
             std::vector<std::shared_ptr<Entity>> &entities) {
  if (direction != generalDirection && (direction == UP || direction == DOWN)) {
  ...
  if (length > 0) {
    std::shared_ptr<Entity> p(new Road(x, 0, z, length, 1, 1, 0.0, 0.0, true,
                                       Entity::Type::NOTCOLLIDABLE));
    // std::shared_ptr<Entity> p(new Road(x,0,z,length,1,1,true,
    // Entity::Type::NOTCOLLIDABLE));
    entities.push_back(p);
    // else{std::shared_ptr<Entity> p(new Road(x,0,z-length,length,1,1,true,
    // Entity::Type::NOTCOLLIDABLE));}
    // entities.push_back(std::shared_ptr<Entity>(new
    // Road(x,0,z-length,length,1,1,true, Entity::Type::NOTCOLLIDABLE)));
  }
}
...
void addBarrier(int x, int z, int length, Direction direction,
  ...
  switch (direction) {
    case UP: {
      ...
      std::shared_ptr<Entity> p(new Barrier(x, 0, z, 2 * length, 1, 1, 10.0,
                                            0.0, true, Entity::Type::COLLIDABLE,
                                            direction));
      entities.push_back(p);
      break;
    }
    ...
    case RIGHT: {
      ...
      std::shared_ptr<Entity> p(new Barrier(x, 0, z, 1, 1, 2 * length, 10.0,
                                            0.0, true, Entity::Type::COLLIDABLE,
                                            direction));
      entities.push_back(p);
      break;
