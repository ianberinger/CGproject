...
void Ramp::draw(Renderer &r,vmml::Matrix4f &modelMatrix){
    ShaderPtr ramShader = r.getObjects()->getShader("ram2");
    ramShader->setUniform("fogColor", fogColor);
    
    r.getModelRenderer()->drawModel("ram2", "camera", modelMatrix*vmml::create_translation(getXYZ()), std::vector<std::string>({ }));
}
