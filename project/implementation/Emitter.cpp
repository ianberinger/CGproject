...
EmitterObject::EmitterObject(float x, float y, float z)
    : Entity(x, y, z, 0.1, 0.1, 0.1, 0.0, 0.0, false,
...
void EmitterObject::draw(Renderer &r, vmml::Matrix4f &modelMatrix) {
  ...
  pShader->setUniform(
      "u_ProjectionMatrix",
      r.getObjects()->getCamera("camera")->getProjectionMatrix());
  pShader->setUniform(
      "u_ModelViewMatrix",
      r.getObjects()->getCamera("camera")->getViewMatrix() * modelMatrix);
  ...
  pShader->registerAttrib("a_pID", 1, GL_FLOAT, sizeof(Particle),
                          offsetof(Particle, pID));
  pShader->registerAttrib("a_pRadiusOffset", 1, GL_FLOAT, sizeof(Particle),
                          offsetof(Particle, pRadiusOffset));
  pShader->registerAttrib("a_pVelocityOffset", 1, GL_FLOAT, sizeof(Particle),
                          offsetof(Particle, pVelocityOffset));
  pShader->registerAttrib("a_pDecayOffset", 1, GL_FLOAT, sizeof(Particle),
                          offsetof(Particle, pDecayOffset));
  pShader->registerAttrib("a_pSizeOffset", 1, GL_FLOAT, sizeof(Particle),
                          offsetof(Particle, pSizeOffset));
  pShader->registerAttrib("a_pColorOffset", 3, GL_FLOAT, sizeof(Particle),
                          offsetof(Particle, pColorOffset));
